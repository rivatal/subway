...
setwd(".")
data <- read.csv("entries_exits_average.csv")
weekend <- read.csv("weekend_averages.csv")
data$X <- NULL
coords <- read.csv("station_ids_coords.csv")
data <- left_join(data, coords)
weekend <- left_join(weekend, coords)

names(weekend)[8] <- "lat"
names(weekend)[9] <- "long"
...
makePlots <- function(data){
  ...
  data <- data %>% mutate(line_name = substr(linename, 0, 1))
  by_station <- group_by(data,station_id) %>%
    summarise(hourly_entries = sum(hourly_entries)/6, hourly_exits = sum(hourly_exits)/6, station = station[1], lat = lat[1] , long = long[1], linename = linename[1])
  ...
  qmplot(long, lat, data = station_volume_day, colour = log(hourly_entries/hourly_exits), size = I(3)) +
    scale_colour_gradient(low = "green", high = "red") +
    geom_text(data = station_volume_day, aes(x=long, y=lat, label=station_id))
  #    scale_colour_gradient(low = "white", high = "black")
  
  #  textxy(station_volume_day$lat, station_volume_day$long, station_volume_day$1)
  
  
  qmplot(long, lat, data = data, fill = line_name, color = line_name, size = hourly_exits)
  
  #weekends <- data %>% filter()
  qmplot(long, lat, data = data, fill = line_name, color = line_name, size = hourly_exits)
}
