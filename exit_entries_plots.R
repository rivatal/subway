...
subway_entries_exits_totals <- group_by(subway_entries_exits,date,day_of_week) %>%
...
subway_entries_exits_means$day_of_week<-factor(subway_entries_exits_means$day_of_week, levels = c("Mon","Tue", "Wed","Thu","Fri", "Sat", "Sun"))
...
ggplot(data=subway_entries_exits_means, aes(x=day_of_week, y=avg, fill=exit_or_entry,show.legend = FALSE)) +
