...
def getdist(name1, name2):
      return int(distanceoffset(turn_terms[t], gtfs_terms[g])) + int(pylev.levenshtein(gtfs_terms[g], turn_terms[t])) + int(isinside(turn_terms[t], gtfs_terms[g])) + int(samewords(turn_terms[t], gtfs_terms[g])) + int(penalize(gtfs_terms[g], turn_terms[t]))
...
gtfs_terms = [] #gtfs
orig_turn = []  #We want to hold onto the originals
orig_gtfs = []
r_best = {}
...
#pattern = re.compile(r'junction')
#Do some formatting, put in a list, put original in an identically indexed list.
for t in turns: 
...
for t in xrange(0, len(turn_terms)):
	...
	#Compute distance:
        tinylist = [getdist(turn_terms[t], gtfs_terms[g]), orig_gtfs[g], gtfs_terms[g], orig_turn[t]]
...
for g in bestmatches:
    f3.write(g.strip() + ", " + str(bestmatches[g][0]).strip().strip('"') + ", " + bestmatches[g][1].strip().strip('"') + ", " + bestmatches[g][3])
